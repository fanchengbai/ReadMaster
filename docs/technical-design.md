...
1. 用户导入一个英文 TXT、EPUB 或文本型 PDF 文件。
2. 系统解析书籍、章节和段落。
3. 用户在阅读器中阅读英文内容。
...
- 默认单个 TXT 文件不超过 10 MB，EPUB 文件不超过 200 MB。
- 不信任上传文件名，由系统生成内部文件名。
- 使用文件哈希辅助识别重复导入。
...
1. 校验 EPUB 压缩包结构、内部路径、最多 10000 个内部文件和最大 200 MB 解压后总体积。
2. 读取 OPF 元数据中的书名与作者。
3. 根据 EPUB spine 获取正确的阅读顺序。
4. 优先使用 EPUB 3 导航目录或 EPUB 2 NCX 目录获取章节标题。
5. 从 XHTML 章节中提取标题、段落、列表和引用文本。
6. 忽略样式、脚本、SVG 和图片内容，保存原始 EPUB 文件用于后续重新解析。

PDF 处理流程：

1. 校验 `%PDF-` 文件头、文件大小、加密状态和页数上限。
2. 读取 PDF 内置的书名与作者元数据。
3. 按原始页面顺序提取文本层，每个包含文字的页面映射为一个阅读章节。
4. 清理空字符、断行连字符和多余空白，再拆分为可定位段落。
5. 保存原始 PDF，保证以后升级解析器时可以重新处理。
6. 纯扫描图片 PDF 暂不执行 OCR，返回明确的“未找到可提取文字”提示。

第一版限制 PDF 不超过 50 MB、2000 页，提取后正文不超过 2000 万字符。PDF 固定布局中的复杂双栏、表格和脚注可能无法完全恢复原阅读顺序，属于后续版面分析能力。
...
当前实现以可见段落为定位依据：阅读器观察正文段落，当主要可见段落发生变化时更新当前位置；切换章节后延迟写入进度，再次打开书籍时恢复章节并滚动到保存的段落。

### 5.6 简单训练

第一版训练直接复用生词与语境快照，不依赖 AI：

- 默认从未标记为“已掌握”的生词中选题，并优先安排错题数较高的词。
- 生词较少时生成语境填词题，将原句中的目标词替换为空格。
- 至少有 4 个带释义的生词时，混合生成四选一释义题。
- 每次提交保存题面、答案、正确答案和作答时间；错误答案同步累计到生词的 `wrong_count`。
- 训练不会自动覆盖用户手动设置的熟悉度。

### 5.7 间隔复习

每个生词保存独立的复习阶段、连续答对次数、上次复习时间和下次复习时间。新保存的词立即到期，训练页只抽取 `next_review_at` 已到达且未标记为“已掌握”的词。

第一版采用固定、可解释的间隔序列：

- 答对：复习阶段前进一级，间隔依次为 1、3、7、14、30、60 天。
- 达到最后一级后，继续答对时保持 60 天间隔。
- 答错：阶段和连续答对次数归零，10 分钟后再次复习。
- 手动标记“已掌握”的词退出复习队列；改回其他状态时沿用原调度时间。

这种规则比完整 SM-2 简单，但在数据量较少的本地 MVP 中更容易理解、测试和调整。积累足够作答数据后，可再引入难度评分和个性化间隔。
...
| review_stage | integer | 当前间隔阶段，初始为 0 |
| consecutive_correct | integer | 连续答对次数 |
| next_review_at | datetime | 下次应复习时间 |
| last_reviewed_at | datetime/null | 最近一次作答时间 |
| note | text/null | 用户备注 |
...
|---|---|---|
| id | UUID | 主键 |
| user_word_id | UUID | 对应生词，删除生词时级联清理 |
| question_type | string | `context_fill` 或 `meaning_choice` |
| prompt_snapshot | text | 作答时的题面快照 |
...
| 方法 | 路径 | 用途 |
|---|---|---|
| POST | `/books/import` | 上传并解析 TXT、EPUB 或文本型 PDF |
| GET | `/books` | 获取书架列表 |
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/review/session` | 按当前生词生成一轮训练 |
| POST | `/review/answer` | 校验并保存作答结果 |
| GET | `/review/stats` | 获取累计作答与正确率 |

训练会同时返回当前到期数量、已安排数量和最近一次下次复习时间，供页面展示今日任务状态。
...
- TXT 编码、章节识别和段落解析
- EPUB 元数据、目录、阅读顺序和 XHTML 正文解析
- EPUB 路径穿越、压缩包体积和损坏文件校验
- 重复文件导入
- 词典缓存
- 生词去重与遇见次数累计
- 阅读进度保存和恢复
...
使用小型英文 TXT 和 EPUB 样本验证：导入、打开、点击单词、保存生词、刷新页面、恢复进度和查看生词库的完整流程。
...
- [x] 可替换词典服务层
- [x] ECDICT 离线英汉词典、词形映射与数据库缓存
- [x] 生词保存、语境快照和状态管理
- [x] 生词库页面

词典数据采用 ECDICT，通过构建脚本将 UTF-8 CSV 转换为独立的 SQLite 数据库。查询顺序为已缓存词条、ECDICT 精确词条、ECDICT 词形映射和内置小型词典兜底。词典原始文件和数据库保存在 `data/dictionaries/`，不提交到 Git；发布词典数据时必须同时保留 ECDICT 的 MIT 许可证。
...
1. 可以导入一份 UTF-8、常见英文编码的 TXT 文件或标准 EPUB 文件。
...
4. 支持 Markdown。
5. 接入段落翻译和长难句分析。
6. 建立词汇、词根、概念和阅读主题之间的关系。
7. 在积累足够学习数据后，再设计个性化能力模型。
